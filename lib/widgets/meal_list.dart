...
class _MealListState extends ConsumerState<MealList> {
  ...
  void _loadMeals() {
    ...
    if (selectedMeals.isNotEmpty) selectedMeals.clear();
    if (availableMealsToAdd.isNotEmpty) availableMealsToAdd.clear();
    if (changedMealPortions.isNotEmpty) changedMealPortions.clear();
  ...
  void didUpdateWidget(MealList oldWidget) {
    ...
    if (widget.selectedDate != oldWidget.selectedDate) {
      logger.d(
        "🌀 Date changed: ${oldWidget.selectedDate} → ${widget.selectedDate}",
      );
      _loadMeals(); // ⬅️ пересоздаём списки
    }
  }
